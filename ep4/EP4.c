...
/**                                                                 **/
/**   <Edgar Henrique de Oliveria Lira>       <12717266>            **/
/**                                                                 **/
/*********************************************************************/
...
if (valor + (c1->valorDaDivida) > dividaMaxima && c1-> tipo == 'C')
return false;
else
{
...
if (valor + (c1->valorDaDivida) > dividaMaximaEspecial && c1 -> tipo == 'E'){
return false;}
else
{
...
else  if (valor > c1->valorDaDivida || valor > c1->valorContaCorrente)
  return false;
...
bool realizarSaque(CLIENTE* c1, int valor) {
 ...
 if (valor <=0)
 return false;
 else if (valor > c1->valorContaCorrente)
 return false;
 else {
c1->valorContaCorrente -=  valor;
return true; 
...
bool adicionarGerente(BANCO* b1, GERENTE* gerente){
  ...
  int x;
  for (x=0;x<b1->numGerentes;x++){
		if (b1->gerentes[x]->pessoa.cpf == gerente->pessoa.cpf) return false;
	}
